...
Una vez que hemos comprobado que la aplicación funciona correctamente, iniciamos una build en local:

```bash
npm run build
```

Y luego podemos iniciar el despliegue sobre la plataforma Netlify:

Iniciamos una sesión en la plataforma y creamos un nuevo proyecto, luego seleccionamos el despliegue desde Github:

![Netlify 1](./images/netlify-1.png)

Posteriormente elegimos el repositorio que vamos a desplegar, si no se muestra, hemos de permitirle el acceso a Github:

![Netlify 2](./images/netlify-2.png)

![Netlify 3](./images/netlify-3.png)

Elegimos la rama sobre la cual se va a desplegar la app, en este caso, lo que ha creado para completar la práctica:
...
*   En el directorio base, escribir el directorio exacto en donde se encuentra alojado el proyecto de NextJS, no tiene porque coincidir con la raiz del repositorio (al principio del desarrollo el proyecto si que estaba en un directorio pero se ha movido a la raíz, entonces este campo se deja vacío).
...
Aquí se muestra una captura de la configuración una vez que ya se ha desplegado el sitio web:
...
![Netlify 8](./images/netlify-8.png)